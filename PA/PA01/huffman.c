...
void traverse(TreeNode *tn, FILE*fp, char *code, char list[], char *vals[], long *counter) {
	char *arrl;
	char *arrr;
	char left[] = "0";
	char right[] = "1";
	...
	traverse((tn)->left, fp, strcat(arrl,left), list,vals,counter);
	traverse((tn)->right,fp, strcat(arrr,right),list,vals,counter);
	if((tn)->ch < 256) {
		//	fprintf(fp,"%c:%s\n",(tn)->ch,code);
